...
This plugin is registered first in ``open_itview5_plugins.cfg`` so its hooks
are installed before subsequent plugins' ``itview_init`` runs, allowing their
...
from itview_console_logger.log_bridge import (
    LogBridge, QtLogHandler, StreamTee)
from itview_console_logger.console_view import ConsoleView


class ItviewConsoleLogger(QtCore.QObject):
