...
class SessionApiCore(QtCore.QObject):
    ...
    def _init(self):
        ...
        self.SIG_PLAYLIST_MODIFIED.emit(self.__fg_playlist_id)
    ...
    def delete_playlists_permanently(self, ids):
        ...
        for playlist_id in ids:
            ...
            commands.flushCachedNode(node_name, False)
        ...
        # correct clip information is obtained.
        self.__update_current_clip()
        self.SIG_PLAYLISTS_MODIFIED.emit()
        return True
    ...
    def __update_bg_retime_node(self, bg_playlist_id):
        ...
        if self.__session.viewport.source_frame_lock == 0:
            ...
            if fg_playlist_len < bg_playlist_len:
                ...
                remaining_frames = fg_playlist_len
                clipped_bg_ids = []

                for clip in bg_clips:
                    frames = clip.get_source_frames()
                    if remaining_frames <= 0:
                        break
                    if len(frames) <= remaining_frames:
                        # Keep whole clip
                        remaining_frames -= len(frames)
                        clipped_bg_ids.append(clip.id)
                    else:
                        # This is the last clip we need, but trimmed
                        self.__set_bg_sync_retime_node(clip.id, frames[:remaining_frames])
                        clipped_bg_ids.append(clip.id)
                        remaining_frames = 0
                bg_playlist.set_active_clips(clipped_bg_ids)
                self.__update_clip_nodes_in_playlist_node(
                    self.__session.get_playlist(self.__session.viewport.bg))
    ...
    def __set_bg_sync_retime_node(self, clip_id, source_frames):
        clip = self.__session.get_clip(clip_id)
        if not clip or not source_frames: return
    ...
    def move_clips_by_offset(self, offset, ids):
        ...
        for playlist_id, clip_ids in to_move.items():
            ...
            for play_order, clip_id in enumerate(playlist.clip_ids):
                ...
                value = play_order + 1
                clip.set_attr_value(attr_id, value)
                all_attr_values.append((playlist.id, clip.id, attr_id, value))
            self.__update_clip_nodes_in_playlist_node(playlist)
            self.SIG_PLAYLIST_MODIFIED.emit(playlist.id)
    ...
    def __update_current_clip(self):
        ...
        if len(clip_frames) == 1:
            ...
            if clip_frame: clip_id = clip_frame[0]
    ...
    def set_current_clip(self, clip_id, frame=None):
        fg_playlist = self.__session.viewport.fg
        active_clip_ids = self.get_active_clips(fg_playlist)
        if clip_id not in active_clip_ids:
            return
        clip = self.__session.get_clip(clip_id)
        if frame is None:
            frame = clip.get_attr_value("key_in")
        source_node = clip.get_custom_attr("rv_source_group")
        global_frame = prop_util.convert_to_global_frame(
            frame, f"{source_node}_source")
        commands.setFrame(global_frame)
        self.__update_current_clip()
        return True
