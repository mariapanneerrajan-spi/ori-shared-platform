...
_IMPORT_NAMESPACE = "itview_cli_args"
    ...
    for folder in plugin_folders:
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                print(
                    f"[itview5 cli] Could not build import spec for {cli_path}",
                    file=sys.stderr)
                continue
            ...
            print(
                f"[itview5 cli] Skipping {cli_path}: import failed: {exc}",
                file=sys.stderr)
            continue

        if not hasattr(module, "add_cmd_line_args"):
            print(
                f"[itview5 cli] {cli_path} has no add_cmd_line_args(parser) "
                f"function - skipping",
                file=sys.stderr)
            continue
...
def build_parser(
    ...
    prog: str = "itview5") -> argparse.ArgumentParser:
    """
...
def parse_and_serialize(
    ...
    try:
        ...
        raise SystemExit(
            f"[itview5 cli] A plugin CLI argument value is not JSON-"
