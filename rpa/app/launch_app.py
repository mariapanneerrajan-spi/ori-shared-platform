...
Invoked from the shell launchers (launch_app.bat / launch_app /
rpa_app / rpa_app.bat) which set up RV_HOME, RV_SUPPORT_PATH, PYTHONPATH,
and RPA_APP_CORE_PLUGINS_CONFIG before running this script.
"""
import os
import sys

# This file lives inside the `rpa_app` package. When invoked as
# `python -m rpa.app.launch_app ...` (how the shell launchers call it)
# the package is already importable. When someone runs the file directly
# from an IDE, make the parent directory importable so `rpa_app.*` imports
# still resolve.
...
def _get_plugin_config_paths():
    """
    Mirror of rpa_app.plugin_path_configs.get() but without pulling in the
    ...
    # Home Directory
    home_cfg = os.path.join(os.path.expanduser("~"), ".rpa_app", config_name)
    if os.path.isfile(home_cfg):
        paths.append(home_cfg)

    # Core Plugins (set by the shell launcher)
    core_cfg = os.environ.get("RPA_APP_CORE_PLUGINS_CONFIG")
...
def main():
    ...
    os.environ[RPA_APP_CLI_ARGS_JSON] = args_json

    # Launch rv with a CLEAN argv - no passthrough. OpenRV's own CLI is
    # now fully replaced by rpa_app's CLI. On Windows os.execv replaces the
