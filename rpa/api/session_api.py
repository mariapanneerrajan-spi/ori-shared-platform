...
class SessionApi(QtCore.QObject):
    ...
    def set_current_clip(self, id:str, frame=None)->bool:
        """
        Set the clip with the given id to be current clip of the Session and if frame
        is given set the current clip at the given frame.
        ...
        Kwargs:
            frame (Optional[int]):
                Optional frame relative to the given clip.

        Returns:
            (bool): True if set False otherwise
        """
        return self.__delegate_mngr.call(self.set_current_clip, [id, frame])
